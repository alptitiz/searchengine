...
public class LinkedDocumentCollection extends DocumentCollection {
	...
	public String getNextUncrawledAddress() {
		DocumentListElement[] elements = this.toArray();

		for (int i = 0; i < elements.length; i++) {
			if (elements[i].getDocument() instanceof DummyLinkedDocument) {
				return ((AbstractLinkedDocument) elements[i].getDocument()).getAddress();
			}
		}
	 ...
	 */
	public boolean addToResultCollection(AbstractLinkedDocument abstractLinkedDocument, String[] outgoing) {
		...
		if (abstractLinkedDocument instanceof LinkedDocument) {
			this.updateIncomingLinks((LinkedDocument) abstractLinkedDocument, outgoing);
		}
	...
	public double[] pageRank(double d, int recDepth) {
		...
		double[] pageRankValues = new double[this.size()];

		for (int i = 0; i < indices.length; i++) {
			double pr = pageRankRec(indices, i, d, recDepth);
			indices[i].setPageRank(pr);
			pageRankValues[i] = pr;
		}
